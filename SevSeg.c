...
void start ( char digitPinsIn[2], char segmentPinsIn[2],  struct SevSeg* sevSegDisplay) {//sets the values of the digitpin and segment pins in the struct, pinmode all of the pins, and turns them all off
  for (int x = 0 ; x < 2 ; x++) {
...
void refresh(struct SevSeg* sevSegDisplay) {//refreshes display and lights up the segments
  ...
  if(i == 0) {
   ...
   for (int j = 0; j < 7; j++) {
     if (sevSegDisplay->digitCodes[i][j] == '1') {
...
void clearSeg(char digitPinsIn[2], char segmentPinsIn[2], struct SevSeg* sevSegDisplay) {//turns pin off
  for (int x = 0 ; x < 2 ; x++) {
...
void findDigits(long numToShow, int digits[], struct SevSeg* sevSegDisplay) { //gets the numToShow, and store each individual digit into the digits array //help on pass by reference for digits
  ...
  const long maxNum = 256;
  const long minNum = 0;
  if (numToShow > maxNum || numToShow < minNum) {
    numToShow = 0;
  }
  if (numToShow < 10) {//if nubmer is between 0 and 9, replace first digit with a 0 NOTE MIGHT NOT NEED THIS
    digits[0] = (long) 0;
    digits[1] = numToShow;
  } else {
    for (char digitNum = 0; digitNum < 2 ; digitNum++) { // otherwise store the 2 digit number into the digits array
      long factor = powersOfBase[1 - digitNum]; //10 or 1
